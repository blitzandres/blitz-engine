...
> **ARCHITECTURE SUPERSEDED** → See `BLITZ_ENGINE_SPEC.md` for architecture and `PROJECT_MAP.md` for repo structure, current status, and implementation order.
...
## Commercialization Notes (Deferred; not active research scope)

```
Option A → user brings their own Claude API key
Option B → subscription if the project ever exits research mode
Option C → freemium if a product is pursued later
...
*architecture correction, local-first hosting update, legal/ethics framing.*
